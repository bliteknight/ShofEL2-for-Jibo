...
Jibo runs a full X server on `:0`. There is no VNC server in the default process list, but `x11vnc` can share the existing display if it is installed in the rootfs.

**Start a VNC server on Jibo (SSH session):**

```bash
x11vnc -display :0 -nopw -listen localhost -rfbport 5900 &
```
...
If `x11vnc` is not present, add it to the rootfs image and write it back with `EMMC_WRITE`.
